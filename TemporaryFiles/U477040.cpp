...
signed main() {
//	freopen("U477040_3.in", "r", stdin);
//	freopen("U477040_3.out","w",stdout);
	...
	for(int i=1; i<=n; i++) {
		...
		for(int j=1; j<=b; j++) {
			cin>>c;
			mon[top].chk.push_back(c);
		}
	}
	for(int i=1; i<=n; i++)
		for(int j=0; j<mon[i].chk.size(); j++)
			mon[pos[mon[i].chk[j]]].d++;

	int flag=1;
	while(flag) {
		flag=0;
		for(int i=1; i<=n; i++)
			if(mon[i].d<=0&&mon[i].cracked==0) {
				mon[i].cracked=1;
				flag=1;
				cnt++;
				for(int j=0; j<mon[j].chk.size(); j++)
					mon[pos[mon[j].chk[j]]].d--;
			}
	}
	if(n-cnt<=0)cout<<"YES";
	else cout<<n-cnt;
