...
double hit(int from, int to)
{
    ...
    return ((un[f].armor_pierce >= un[t].armor) ? 1 : 0.4) *
           (((un[f].Soft_Atk * (sd[from].side == 1 ? 1 : 0.8) - un[t].defence * (sd[to].side == 1 ? 0.6 : 1)) * (un[f].Soft_Atk * (sd[from].side == 1 ? 1 : 0.8) >= un[t].defence * (sd[to].side == 1 ? 0.6 : 1)) +
             min(un[t].defence * (sd[to].side == 1 ? 0.6 : 1), un[f].Soft_Atk * (sd[from].side == 1 ? 1 : 0.8)) * 0.3) *
                un[t].Soft_HP +
            ((un[f].Hard_atk * (sd[from].side == 1 ? 1 : 0.8) - un[t].defence * (sd[to].side == 1 ? 0.6 : 1)) * (un[f].Hard_atk * (sd[from].side == 1 ? 1 : 0.8) >= un[t].defence * (sd[to].side == 1 ? 0.6 : 1)) +
             min(un[t].defence * (sd[to].side == 1 ? 0.6 : 1), un[f].Hard_atk * (sd[from].side == 1 ? 1 : 0.8)) * 0.3) *
                un[t].Hard_HP);
}
double HIT(int f, int t)
{
    return ((un[f].armor_pierce >= un[t].armor) ? 1 : 0.4) *
           (((un[f].Soft_Atk - un[t].defence) * (un[f].Soft_Atk >= un[t].defence) +
             min(un[t].defence, un[f].Soft_Atk) * 0.3) *
                un[t].Soft_HP +
            ((un[f].Hard_atk - un[t].defence) * (un[f].Hard_atk >= un[t].defence) +
             min(un[t].defence, un[f].Hard_atk) * 0.3) *
                un[t].Hard_HP);
}
void calc(int id)
{
    ...
    for (int i = 1; i <= wf[id].fltop[0]; ++i)
    {
        if (sd[wf[id].frontline[0][i]].hp < 0)
            del(wf[id].frontline[0][i]);
    }
    for (int i = 1; i <= wf[id].fltop[1]; ++i)
    {
        if (sd[wf[id].frontline[1][i]].hp < 0)
            del(wf[id].frontline[1][i]);
    }
}
bitset<N> vis;
int main()
{
    ...
    while (1)
    {
        ...
        for (int id = 1; id <= n;)
        {
            ...
            display();
            cout << endl
                 << "Now:" << id << " " << plr[id].name << "(" << plr[id].team << ")" << " eco:" << plr[id].eco << "/" << plr[id].eco_per_rnd << endl;
            cin >> com;
            if (plr[id].eco < 0)
            {
                id++;
            ...
            {
                cin >> inp;
                if (loc[inp].ownteam != plr[id].team || plr[id].eco < ter[loc[inp].terr].cost_cstr * init_consq[(int)floor(loc[inp].eco)])
                    continue;
            ...
            else if (com == "deploy" || com == "dpl")
            {
                ...
                cin >> inp;
                if (plr[id].eco < un[inp].cost || inp > sod_typesize)
                    continue;
                sd[++sdtop].to = inp;
            ...
            {
                cin >> inp;
                if (sd[inp].belong != id || loc[sd[inp].nowloc].ownteam != plr[id].team)
                    continue;
                if (sd[inp].infl)
                {
