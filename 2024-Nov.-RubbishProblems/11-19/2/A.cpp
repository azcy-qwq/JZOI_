...
const int pcc=7777777;
const int N=20;
ll n,x;
void Add(ll &x,ll y){x=(x+y>=pcc?x+y-pcc:x+y);}
inline ll mul(ll x,ll y){return 1ll*x*y%pcc;}
struct Matrix
{
	ll c[N][N];
	...
	friend Matrix operator *(Matrix x,Matrix y)
	{
		...
		for(int k=1;k<=n;++k)
			for(int i=1;i<=n;++i)
				for(int j=1;j<=n;++j)
					Add(res.c[i][j],mul(x.c[i][k],y.c[k][j]));
		return res; 
	}
}F,A;
void ksm(ll k)
