...
const int N=5e5+10;
unsigned long long n,ans,cnt;
constexpr unsigned int solve(){
    unsigned int gg=0;
    for(unsigned long long i=1;i<=1e17;i+=gg){
        gg+=__builtin_popcount(i);
    }return gg;
}
int main(){
//ios::sync_with_stdio(0);
    // cin>>n;
    n=1e17;
    // for(unsigned long long i=1;i<=n;i+=ans){
